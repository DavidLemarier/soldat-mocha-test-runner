...
export default function createRunner (options = {}, callback) {
  ...
  return ({testPaths, buildSoldatEnvironment, buildDefaultApplicationDelegate, logFile, headless}) => {
    function runTests (testPaths, resolve) {
      ...
      // so that test authors can just `require 'soldat/test'`
      global.buildSoldatEnvironment = function (params = {}) {
        let defaultParams = {
          applicationDelegate,
          window,
          document,
        ...
        return buildSoldatEnvironment(Object.assign(defaultParams, params))
      }

      if (options.globalSoldat) {
